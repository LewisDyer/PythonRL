...
class GameMap:
    ...
    def create_room(self, room):
        # make tiles in rectangle passable - make sure to leave walls!
    ...
    def place_entities(self, room, entities, max_monsters_per_room, max_items_per_room):
        ...
        no_monsters = randint(0, max_monsters_per_room)
        no_items = randint(0, max_items_per_room)

        for i in range(no_monsters):
        ...
        for i in range(no_items):
            ...
            if not any([entity for entity in entities if entity.x == x and entity.y == y]):
                ...
                if item_chance < 70:
                    ...
                    item_component = Item(use_function = cast_fireball, targeting = True, targeting_message =
                                          Message('Left click a target tile for the fireball, '
                                                  'or right click to cancel.', libtcod.light_cyan), damage=12, radius=3)
                    item = Entity(x, y, '#', libtcod.red, 'Fireball Scroll', render_order = RenderOrder.ITEM,
                                  item = item_component)
                elif item_chance < 90:
                    item_component = Item(use_function = cast_confuse, targeting = True, targeting_message =
